        ...
        for dataset_name in datasets_name:
            ...
            for repeat in range(args.repeat_num):
                ...
                for i, data in enumerate(data_list):
                    data = preselect_anchor(data, layer_num=args.layer_num, anchor_num=args.anchor_num, device='cpu')
                    # data = data.to(device)
                    data_list[i] = data
                ...
                for epoch in range(args.epoch_num):
                    ...
                    for id, data in enumerate(data_list[:effective_len]):
                        ...
                        edge_mask_train = np.concatenate(
                            (data['mask_link_positive_train'], data['mask_link_negative_train']),
                            axis=-1)
                        nodes_first = torch.index_select(out, 0,
                                                         torch.from_numpy(edge_mask_train[0, :]).long().to(device))
                        nodes_second = torch.index_select(out, 0,
                                                          torch.from_numpy(edge_mask_train[1, :]).long().to(device))
                    ...
                    if epoch % args.epoch_log == 0:
                        ...
                        for id, data in enumerate(data_list):
                            ...
                            pred = torch.sum(nodes_first * nodes_second, dim=-1)
                            label_positive = torch.ones([data['mask_link_positive_train'].shape[1], ], dtype=pred.dtype)
                            label_negative = torch.zeros([data['mask_link_negative_train'].shape[1], ],
                                                         dtype=pred.dtype)
                            label = torch.cat((label_positive, label_negative)).to(device)
                            ...
                            edge_mask_val = np.concatenate(
                                (data['mask_link_positive_val'], data['mask_link_negative_val']),
                                axis=-1)
                            nodes_first = torch.index_select(out, 0,
                                                             torch.from_numpy(edge_mask_val[0, :]).long().to(device))
                            nodes_second = torch.index_select(out, 0,
                                                              torch.from_numpy(edge_mask_val[1, :]).long().to(device))
                            pred = torch.sum(nodes_first * nodes_second, dim=-1)
                            ...
                            nodes_first = torch.index_select(out, 0,
                                                             torch.from_numpy(edge_mask_test[0, :]).long().to(device))
                            nodes_second = torch.index_select(out, 0,
                                                              torch.from_numpy(edge_mask_test[1, :]).long().to(device))
                            pred = torch.sum(nodes_first * nodes_second, dim=-1)
                        ...
                        writer_test.add_scalar('repeat_' + str(repeat) + '/emb_min_' + dataset_name, emb_norm_min,
                                               epoch)
                        writer_test.add_scalar('repeat_' + str(repeat) + '/emb_max_' + dataset_name, emb_norm_max,
                                               epoch)
                        writer_test.add_scalar('repeat_' + str(repeat) + '/emb_mean_' + dataset_name, emb_norm_mean,
                                               epoch)
            ...
            with open('results/{}_{}_{}_layer{}_approximate{}.txt'.format(args.task, args.model, dataset_name,
                                                                          args.layer_num, args.approximate), 'w') as f:
